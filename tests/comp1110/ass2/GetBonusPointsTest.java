...
public class GetBonusPointsTest {
    ...
    private final String[][] endGameStates = {
    ...
    public void testGetBonusPoints() {
        for (int i = 0; i < endGameStates.length; i++) {
            int playerA = Azul.getBonusPoints(endGameStates[i], 'A');
            int playerB = Azul.getBonusPoints(endGameStates[i], 'B');
            assertEquals(scores[i][0], playerA, "Azul.getBonusPoints({" + endGameStates[i][0] + "," + endGameStates[i][1] + "},\"A\"");
            assertEquals(scores[i][1], playerB, "Azul.getBonusPoints({" + endGameStates[i][0] + "," + endGameStates[i][1] + "},\"B\"");
        }
    }
}
